...
export default function HomeScreen4({ repoDatas}: { repoDatas: Partial<IRespositories>[]}) {
  ...
  const [themeSwitcher, setThemeSwitcher] = useState('light')
  ...
  return (
    <div className="h-screen flex flex-col items-center pt-20 ">
      ...
      <div className="flex p-4 w-full h-full md:max-w-[60%] justify-center">
        <div className={`${themeSwitcher === 'dark' ? 'border border-gray-700' : 'border'} w-full h-[90%] md:h-[80%] relative rounded-xl overflow-hidden shadow-xl bg-secondary`}>
          <div className={`${themeSwitcher === 'dark' ? 'border-b border-gray-600' : 'border-b'} h-[30px] absolute z-10 min-w-full flex items-center justify-between px-2 bg-danger/80 backdrop-blur-xl shadow-md rounded-t-xl`}>
            ...
            <div className="flex w-full justify-center">
              <small className={`${themeSwitcher === 'dark' ? 'text-slate-300 bg-gray-500/60 border border-slate-500' : 'text-slate-500 bg-gray-100/60 border border-slate-300'} text-center  w-full mx-2 md:w-[40%] rounded-md`}>{menus[showMenu].title}</small>
              <div
                onClick={() => showElementByScroll(-window.innerWidth / 2, false)}
                className={`${themeSwitcher === 'dark' ? 'text-slate-300 bg-gray-500/60 border border-slate-500' : 'text-slate-500 bg-gray-100/60 border border-slate-300'} cursor-pointer px-1 flex justify-center items-center rounded-md mr-2`}>
              ...
              <div
                ...
                className={`${themeSwitcher === 'dark' ? 'text-slate-300 bg-gray-500/60 border border-slate-500' : 'text-slate-500 bg-gray-100/60 border border-slate-300'} cursor-pointer px-1 flex justify-center items-center rounded-md`}>
